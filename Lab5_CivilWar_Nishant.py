...
if __name__ == "__main__":
    ...
    converted_data = r.read_rda('Ch9_Exercise1_Civil_war.RData')
    ...
    model = smf.ols(formula='InternalConflict ~ LaggedGDPGrowth', data=df)
    results = model.fit()
    # Print the summary
    r_summary(results, formula='InternalConflict ~ LaggedGDPGrowth', data_name='df')

    # add control variables for initialGDP, democracy, mountains, ethnic and religious fractionalization
    model = smf.ols(formula='InternalConflict ~ LaggedGDPGrowth + InitialGDP + Democracy + Mountains + EthnicFrac + ReligiousFrac', data=df)
    results = model.fit()
    # Print the summary
    r_summary(results, formula='InternalConflict ~ LaggedGDPGrowth + InitialGDP + Democracy + Mountains + EthnicFrac + ReligiousFrac', data_name='df')

    # add control variables for LaggedRainfallGrowth, democracy, mountains, ethnic and religious fractionalization
    model = smf.ols(formula='InternalConflict ~ LaggedRainfallGrowth + InitialGDP + Democracy + Mountains + EthnicFrac + ReligiousFrac', data=df)
    results = model.fit()
    # Print the summary
    r_summary(results, formula='InternalConflict ~ LaggedRainfallGrowth + InitialGDP + Democracy + Mountains + EthnicFrac + ReligiousFrac', data_name='df')

    from linearmodels.iv import IV2SLS
    # The structure is: 'dependent ~ exog_controls + [endog_variable ~ instrument_variable]'
    formula = 'InternalConflict ~ 1 + InitialGDP + Democracy + Mountains + EthnicFrac + ReligiousFrac + [LaggedGDPGrowth ~ LaggedRainfallGrowth]'
    # Fit the model using two-stage least squares (2SLS)
    iv_1 = IV2SLS.from_formula(formula, data=df).fit()
    # Print the summary
    print(iv_1.summary)

    print(df[['LaggedGDPGrowth','LaggedRainfallGrowth']].corr())
    v = df.groupby('country_code')["LaggedGDPGrowth"].std()
    df.dropna(inplace=True)  # Ensure no NaN values before fitting the model
    df['country_code'] = df['country_code'].astype('category')
    base = df['country_code'].cat.categories[0]  # Get the base category
    print(base)
    formula = "InternalConflict ~ 1 + InitialGDP + Democracy + Mountains + EthnicFrac + ReligiousFrac + C(country_code, Treatment('{base}')) + [LaggedGDPGrowth ~ LaggedRainfallGrowth]"
    # Fit the model using two-stage least squares (2SLS)
    iv_1 = IV2SLS.from_formula(formula, data=df).fit()
    # Print the summary
    print(iv_1.summary)
